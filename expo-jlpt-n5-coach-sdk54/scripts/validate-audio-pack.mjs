...
const extraFiles = fs.existsSync(audioDir)
  ? fs.readdirSync(audioDir)
      ...
      .filter((name) => !seenIds.has(name.replace(/\.wav$/, '')))
  : [];
...
if (extraFiles.length > 0) console.log(`Extra WAV files not in manifest: ${extraFiles.slice(0, 8).join(', ')}`);
...
if (strict && missing.length > 0) console.log('Strict mode: missing WAV files are blocking.');
